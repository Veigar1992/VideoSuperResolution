...
def main(argv):
    ...
    if data_mode == 'Vid4':
        ...
        test_dataset_folder = '../datasets/HDR/test/pngs/SDR_540p'
    else:
        if stage == 1:
    ...
    curr_id = int(gpu_id)
    if curr_id != 7:
        subfolder_l_curr = subfolder_l[curr_id * 6 : (curr_id+1) * 6]
    elif curr_id == 7:
        subfolder_l_curr = subfolder_l[curr_id * 6 :]
    else:
        subfolder_l_curr = None
        print('???')
    # print(subfolder_l_curr)
    # exit(0)
    # for each subfolder
    for subfolder in subfolder_l_curr:
        ...
        for img_idx, img_path in enumerate(img_path_l):
            ...
            logger.info('{:3d} - {:25} '.format(img_idx, img_name))
            #

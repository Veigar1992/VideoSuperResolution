    ...
    for file in $(ls ./videos/$folder); do
            tmpname="$(basename -- $file)"  
                sname=${tmpname##*-}
                    ...
                    # mkdir -p $png_path/$folder/$bname
                    #for %a in ("*.mp4") do ffmpeg -i "%a" -vsync 0 "数据集路径\%~na%4d.png" -y
                    echo ffmpeg -i ./videos/$folder/$file -vsync 0 ./$png_path/$folder/${bname}/%08d.png -y > ${log_name} 2>&1
                    ffmpeg -i ./videos/$folder/$file -vsync 0 ./$png_path/$folder/${bname}/%08d.png -y > ${log_name} 2>&1
